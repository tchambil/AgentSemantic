...
// console.log(data);
    ...
    showUserIdentityInput: function(e) {
      ...
      document.body.insertAdjacentHTML('beforeend', '<aside id="user-identity-input" class="do on"><button class="close" title="Close">âŒ</button><h2>Sign in with WebID</h2><label>HTTP(S) IRI</label> <input id="webid" type="text" placeholder="http://csarven.ca/#i" value="" name="webid"/> <button class="signin">Sign in</button></aside>');
...
// console.log(i);
      ...
      if (stylesheets.length > 0) {
        ...
        for (var i = 0; i < stylesheets.length; i++) {
          ...
          if(stylesheet.matches('[rel~="alternate"]')) {
            s += '<li><button title="Change to â€˜' + view + 'â€™ view">' + view + '</button></li>';
          }
          else {
    ...
    showEmbedData: function(node) {
      ...
      var eventEmbedData = function(e) {
        ...
        var embedMenu = '<aside id="embed-data-entry" class="do on tabs"><button class="close" title="Close">âŒ</button>\n\
        <h2>Embed Data</h2>\n\
    ...
    showToC: function() {
      ...
      if (sections.length > 0) {
        ...
        s = '<aside id="toc" class="do on' + sortable + '"><button class="close" title="Close">âŒ</button></aside>';
        ...
        ids[i].addEventListener('mouseenter', function(e){
          ...
          if (!fragment && e.target.parentNode.nodeName.toLowerCase() != 'aside'){
            ...
            switch(e.target.nodeName.toLowerCase()) {
              default:    sign = 'ðŸ”—'; break;
              case 'section':
                switch (e.target.id) {
                  default:          sign = 'Â§'; break;
                  case 'references':    sign = 'â˜›'; break;
                  case 'acknowledgements':  sign = 'â˜º'; break;
                  case 'results':       sign = 'âˆž'; break;
                  case 'related-work':    sign = 'âŒ˜'; break;
                  case 'keywords':      sign = 'ðŸ·'; break;
                  case 'conclusions':     sign = 'âˆ´'; break;
                }
                break;
              case 'aside':   sign = 'â€ '; break;
              case 'p':     sign = 'Â¶'; break;
              case 'pre':   sign = 'ðŸ–©'; break;
              case 'nav':   sign = 'â˜›'; break;
              case 'dfn':   sign = 'ðŸ“‡'; break;
              case 'table':   sign = 'ð„œ'; break;
              case 'figure':  sign = 'â¦'; break;
              case 'img':   sign = 'ðŸ–¼'; break;
              case 'video':   sign = 'ðŸŽž'; break;
              case 'audio':   sign = 'ðŸ”Š'; break;
              case 'footer':  sign = 'â¸™'; break;
            }
...
';
    ...
    shareResource: function(e, iri) {
      ...
      if(!(DO.C.User.Graph && ((DO.C.User.Knows && DO.C.User.Knows.length > 0) || (DO.C.User.Graph.owlsameAs && DO.C.User.Graph.owlsameAs._array.length > 0)))) {
        ...
        noContactsText = '<p>No contacts with an <i class="fa fa-inbox"></i> Inbox found. Acquire <i class="fa fa-thermometer-empty"></i> cool friendsâ€½</p><p>Optionally enter targets individually:</p>';
      }
        ...
        DO.U.getContacts(url).then(
          function(contacts) {
            if(contacts.length > 0) {
              ...
              e.target.parentNode.innerHTML = 'No contacts with <i class="fa fa-inbox"></i> Inboxes found. Acquire <i class="fa fa-thermometer-empty"></i> cool friendsâ€½</p><p>Optionally enter targets individually:</p>';
            }
          },
          function(reason) {
             console.log(reason);
...
// console.log(iri.toString());
    ...
    showResourceBrowser: function() {
      ...
      var browserHTML = '<aside id="resource-browser" class="do on"><button class="close" title="Close">âŒ</button><h2>Resource Browser</h2></aside>';
    ...
    openDocument: function(e) {
      ...
      document.body.insertAdjacentHTML('beforeend', '<aside id="open-document" class="do on"><button class="close" title="Close">âŒ</button><h2>Open Document</h2></aside>');
...
// console.log(reason);
    ...
    createNewDocument: function(e) {
      e.target.disabled = true;
      document.body.insertAdjacentHTML('beforeend', '<aside id="create-new-document" class="do on"><button class="close" title="Close">âŒ</button><h2>Create New Document</h2></aside>');
...
// console.log(baseURLSelectionChecked);
    ...
    saveAsDocument: function(e) {
      e.target.disabled = true;
      document.body.insertAdjacentHTML('beforeend', '<aside id="save-as-document" class="do on"><button class="close" title="Close">âŒ</button><h2>Save As Document</h2></aside>');
    ...
    viewSource: function(e) {
      e.target.disabled = true;
      document.body.insertAdjacentHTML('beforeend', '<aside id="source-view" class="do on"><button class="close" title="Close">âŒ</button><h2>Source</h2><textarea id="source-edit" rows="24" cols="80"></textarea><p><button class="create">Update</button></p></aside>');
...
WHERE {\n\
  ...
  ?observation ?propertyMeasure ?obsValue .\n\
    ...
    getReferenceLabel: function(motivatedBy) {
      ...
      switch(motivatedBy) {
        default: break;
        case 'oa:assessing':  s = 'âœª'; break;
        case 'oa:commenting': s = 'ðŸ—¨'; break;
        case 'oa:bookmark':   s = 'ðŸ”–'; break;
        case 'oa:replying':   s = 'ðŸ’¬'; break;
        case 'oa:describing': s = 'â€»'; break;
      }
